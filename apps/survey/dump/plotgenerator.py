...
#         # Create a horizontal bar chart
#         plt.barh(categories, counts, color='skyblue', label='Count')
#         plt.xlabel('Count')
#         plt.ylabel('Alignment Level')
#         plt.title('Alignment of Academic Specialization with Job Responsibilities, by Job Satisfaction')
...
# # def generate_role_satisfaction_by_academic_specialization():
# #     try:
# #         # q1: academic specialization (CS or IT)
# #         # q3: how well did your academic specialization prepare you for your current job role
# #         # Generate data for satisfaction levels with academic specialization preparation (replace this with your actual data)
# #         satisfaction_data = Survey.objects.values('q3', 'q1').annotate(count=Count('q3'))

# #         # Define custom sorting order for academic specialization and satisfaction levels
# #         academic_specializations_order = ['CS', 'IT']
# #         satisfaction_levels_order = ['Not at All', 'Poorly', 'Neutral', 'Well', 'Very Well']

# #         # Extract data for the grouped bar chart
# #         categories = sorted(list(set(entry['q3'] for entry in satisfaction_data)), key=lambda x: satisfaction_levels_order.index(x))
# #         academic_specializations = sorted(academic_specializations_order)

# #         # Create a grouped bar chart
# #         bar_width = 0.15  # Adjust as needed
# #         index = np.arange(len(categories))
# #         labels = [f'{specialization} - {satisfaction_level}' for satisfaction_level in satisfaction_levels_order]

# #         for i, specialization in enumerate(academic_specializations):
# #             specialization_counts = [entry['count'] if entry['q1'] == specialization and entry['q3'] == satisfaction_level else 0 for entry in satisfaction_data for satisfaction_level in satisfaction_levels_order]
# #             plt.bar(index + i * bar_width, specialization_counts, bar_width, label=[f'{specialization} - {satisfaction_level}' for satisfaction_level in satisfaction_levels_order])

# #         plt.xlabel('Satisfaction Level')
# #         plt.ylabel('Count')
# #         plt.title('Satisfaction Levels with Academic Specialization Preparation, by Academic Specialization')
# #         plt.xticks(index + (bar_width * (len(academic_specializations) - 1)) / 2, categories)

# #         # Add legend for each academic specialization
# #         plt.legend(title='Academic Specialization - Satisfaction Level', bbox_to_anchor=(1, 1), loc='upper left')
